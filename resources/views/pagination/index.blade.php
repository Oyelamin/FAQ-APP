...
<nav class="pagination is-rounded" style="margin-top:-25px;float:right;border-radius:10px;padding:5px;background:rgb(233, 233, 233);color: white;-webkit-border-radius: 10px;-moz-border-radius: 10px;" role="navigation" aria-label="pagination">
    <ul class="pagination">
        ...
        @if ($paginator->onFirstPage())
            <li class="disabled pagination-previous"><span>Previous Page</span></li>
        @else
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li style="background: silver;border:1px solid silver;" class="pagination-link active is-current is-danger"><span>{{ $page }}</span></li>
                    @else
